...
pub fn load_and_serve_event(
    ...
    *current_year = year;
    ...
    server.fn_handler(format!("/{}", year).as_str(), Method::Get, move |request| {
    ...
    for (day, sol) in aoc {
        ...
        server.fn_handler(format!("/{}/day/{}", year, day).as_str(), Method::Get, move |request| {
            let mut response = request.into_ok_response()?;
    ...
    for (day, sol) in aoc {
        ...
        server.fn_handler(format!("/{}/day/{}", year, day).as_str(), Method::Post, move |mut request| {
            let level;
            let puzzle_upload;
            let puzzle_answer;
            ...
            match parse_http_request(&mut request) {
                ...
                None => {
                    request.into_status_response(413)?.write_all("Wrong request.".as_bytes())?;
                    return Ok(());
                }
            }
            ...
            unsafe { minimum_free_heap_size_before = esp_get_minimum_free_heap_size(); }
            ...
            unsafe { minimum_free_heap_size_after = esp_get_minimum_free_heap_size(); }
